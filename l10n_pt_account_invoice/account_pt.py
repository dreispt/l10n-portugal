...
class AccountTax(models.Model):
    ...
    def _unit_compute(self, cr, uid, taxes, price_unit, product=None, partner=None, quantity=0):
        data = super(account_pt_account_tax, self)._unit_compute(cr, uid, taxes, price_unit, product, partner, quantity)
        for dict in data:
            if 'id' in dict:
    ...
    def _unit_compute_inv(self, cr, uid, taxes, price_unit, product=None, partner=None):
        data = super(account_pt_account_tax, self). _unit_compute_inv(cr, uid, taxes, price_unit, product, partner)
        for dict in data:
            account_debit_id = self.browse(cr, uid, dict['id'])
            if account_debit_id:
                dict.update({'account_debit_id':account_debit_id.id,})
...
class AccountJournal(models.Model):
    ...
    def _check_unique_stock_journal(self):
        ...
        stock_journals = self.search([('company_id', 'in', company_ids), ('stock_journal', '=', True)])
